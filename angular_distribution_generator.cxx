...
void angular_distribution_generator()
{
...
7.5875,7.71,7.8275,7.94,8.0475,8.15}; //cm
    ...
    TFile write("analysis_output/angular_distribution_histograms.root", "recreate");
            ...
            for(int j=0; j<SX3Mul; j++)
            {
                if(SX3Det[j] <= 11 && SX3Strip[j] <= 3 && tdcIC >= 900 && tdcIC <= 1300) //No weird events with wrong channels make it through
                {
                    ...
                    for(int l=1; l<=number_of_SX3_bins; l++)
                    {
                        ...
                        {
                            Excitation_AngularBin_SX3[l]->Fill(corrected_excitation);
                        }
                    }
            ...
            for(int j=0; j<QQQ5Mul; j++)
            {
                if(QQQ5Det[j] <= 4 && QQQ5Ring[j] <= 31 && tdcIC >= 900 && tdcIC <= 1300) //Se82_locus_IC->IsInside(icE, icdE)
                {
                    ...
                    for(int l=1; l<=number_of_QQQ5_bins; l++)
                    {
                        ...
                        {
                            Excitation_AngularBin_QQQ5[l]->Fill(corrected_excitation);
                        }
                    }
