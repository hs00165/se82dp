...
*/
    ...
    */
    ...
    Chain->Add("/mnt/se82dp_2/cal_merged/calibrated_calibration_runs/Run202.root");     //alpha_Ecal_60deg01.ldf
    //Chain->Add("/mnt/se82dp_2/cal_merged/calibrated_calibration_runs/Run203.root");     //alpha_Ecal_120deg01.ldf
    //Chain->Add("/mnt/se82dp_2/cal_merged/calibrated_calibration_runs/Run204.root");     //alpha_Ecal_180deg02.ldf
    //Chain->Add("/mnt/se82dp_2/cal_merged/calibrated_calibration_runs/Run205.root");     //alpha_Ecal_240deg02.ldf
    //Chain->Add("/mnt/se82dp_2/cal_merged/calibrated_calibration_runs/Run206.root");     //alpha_Ecal_300deg01.ldf
    ...
    TFile write("cal/BSX3_cal_hists.root", "recreate");
    //Output .dat file for calibration parameters
    ofstream BSX3UpEncalParametersFile;
    BSX3UpEncalParametersFile.open ("cal/cal_params/BSX3UpEncal_Parameters_OUTPUT.txt");
    ofstream BSX3DnEncalParametersFile;
    BSX3DnEncalParametersFile.open ("cal/cal_params/BSX3DnEncal_Parameters_OUTPUT.txt");
    ...
    TH2D* En_vs_Ch_UP = new TH2D("En_vs_Ch_UP", "En_vs_Ch_UP", 150, 0, 150, 1000, 0, 10000);
    TH2D* En_vs_Ch_DN = new TH2D("En_vs_Ch_DN", "En_vs_Ch_DN", 150, 0, 150, 1000, 0, 10000);


    //Creating the histograms 
    char E_spec_HistName[512];
    TH1D *BSX3_E_spec[2][12][4];
    for(int u=0;u<2;u++)
    {
        for(int d=0;d<12;d++)
        {
            for(int s=0;s<4;s++)
            {
                if(u==1) sprintf(E_spec_HistName,"BSX3Up_En_Det%dStrip%d",d,s);
                if(u==0) sprintf(E_spec_HistName,"BSX3Dn_En_Det%dStrip%d",d,s);

                BSX3_E_spec[u][d][s] = new TH1D(E_spec_HistName,E_spec_HistName,2000,0,4000);
            }
        }  
    }

    char ADC_spec_HistName[512];
    TH1D *BSX3_ADC_spec[2][12][4];
    for(int u=0;u<2;u++)
    {
        for(int d=0;d<12;d++)
        {
            for(int s=0;s<4;s++)
            {
                if(u==1) sprintf(ADC_spec_HistName,"BSX3Up_ADC_Det%dStrip%d",d,s);
                if(u==0) sprintf(ADC_spec_HistName,"BSX3Dn_ADC_Det%dStrip%d",d,s);

                BSX3_ADC_spec[u][d][s] = new TH1D(ADC_spec_HistName,ADC_spec_HistName,1000,1000,4000);
            }
        }  
    }
	...
	for ( unsigned long long int i=0; i<nEntries; i++ )
    {
        ...
        if(SX3Mul >= 1)
        {
            ...
            {
                if(SX3Det[j] <= 12 && SX3Sector[j] <= 5)
                {
                    BSX3_En = (SX3SectorADC[j] * BSX3_Encal[SX3Upstream[j]][SX3Det[j]][SX3Sector[j]][1]) + BSX3_Encal[SX3Upstream[j]][SX3Det[j]][SX3Sector[j]][0];

                    //Filling individual 1D histograms of SX3SectorEnergy
                    BSX3_E_spec[SX3Upstream[j]][SX3Det[j]][SX3Sector[j]]->Fill(BSX3_En);

                    //Filling individual 1D histograms of SX3SectorADC
                    BSX3_ADC_spec[SX3Upstream[j]][SX3Det[j]][SX3Sector[j]]->Fill(SX3SectorADC[j]);

                    //Filling a 2D Energy vs Channel histogram
                    if(SX3Upstream[j] == 1) En_vs_Ch_UP->Fill((SX3Det[j]*4) + SX3Sector[j], BSX3_En);
                    if(SX3Upstream[j] == 0) En_vs_Ch_DN->Fill((SX3Det[j]*4) + SX3Sector[j], BSX3_En);
    ...
    for(int d=0;d<12;d++)
    {
        for(int s=0;s<4;s++)
        {
            ...
            BSX3UpEncalParametersFile << d << setw(15) << "0" << setw(15) << s << setw(15) << cal_fit->GetParameter(0) << setw(15) << cal_fit->GetParameter(1) << endl;
    ...
    for(int d=0;d<12;d++)
    {
        for(int s=0;s<4;s++)
        {
            BSX3_ADC_spec[0][d][s]->Draw();

            // Searching the histogram for the largest 5 peaks, with a threshold of 30%
            TSpectrum* spec = new TSpectrum(5);
            Int_t nPeaks = spec->Search(BSX3_ADC_spec[0][d][s],1, "", 0.3);
            Double_t *raw_peak_pos = spec->GetPositionX();
            ...
            // 8784 keV Peak
            TF1* peak8784 = new TF1 ( "peak8784","gaus(0)",fit_min_range[4], fit_max_range[4] );
            BSX3_ADC_spec[0][d][s]->Fit ( "peak8784", "r" );
            // 6778 keV Peak
            TF1* peak6778 = new TF1 ( "peak6778","gaus(0)",fit_min_range[3], fit_max_range[3] );
            BSX3_ADC_spec[0][d][s]->Fit ( "peak6778", "r" );
            // 6288 keV Peak
            TF1* peak6288 = new TF1 ( "peak6288","gaus(0)",fit_min_range[2], fit_max_range[2] );
            BSX3_ADC_spec[0][d][s]->Fit ( "peak6288", "r" );
            // 5685 keV Peak
            TF1* peak5685 = new TF1 ( "peak5685","gaus(0)",fit_min_range[1], fit_max_range[1] );
            BSX3_ADC_spec[0][d][s]->Fit ( "peak5685", "r" );
            // 5423 keV Peak
            TF1* peak5423 = new TF1 ( "peak5423","gaus(0)",fit_min_range[0], fit_max_range[0] );
            BSX3_ADC_spec[0][d][s]->Fit ( "peak5423", "r" );

            // Linear regression of measured ADC peak values
            // against known alpha energies
            calPlot[0][d][s] = new TGraph(5,raw_peak_pos, cal_peak_pos);
            sprintf(calPlot_GraphName,"calPlotUp_Det%dStrip%d",d,s);
            calPlot[0][d][s]->SetName(calPlot_GraphName);
            calPlot[0][d][s]->SetMarkerStyle(23);
            TF1* cal_fit = new TF1 ( "cal_fit", "pol1", 0, 5000);
            calPlot[0][d][s]->Fit ( "cal_fit", "r" );
            cout << "p0:   " << cal_fit->GetParameter(0) << "    p1:    " << cal_fit->GetParameter(1) << endl; 
            calPlot[0][d][s]->Write();

            // Writing out the calibration parameters to file:
            // Det         Ring        Intercept     Slope
            BSX3DnEncalParametersFile << d << setw(15) << "0" << setw(15) << s << setw(15) << cal_fit->GetParameter(0) << setw(15) << cal_fit->GetParameter(1) << endl;
