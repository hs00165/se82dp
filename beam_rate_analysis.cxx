...
7.5875,7.71,7.8275,7.94,8.0475,8.15};
    ...
    TFile write("analysis_output/beam_rate_analysis.root", "recreate");
    ...
    TH1D* rate_hist = new TH1D("rate_hist", "rate_hist", 15000, 0, 15000);
    TH1D*  elastics_rate_hist = new TH1D("elastics_rate_hist", "elastics_rate_hist", 15000, 0, 15000);

    TH1D* run_marker_hist = new TH1D("run_marker_hist", "run_marker_hist", 15000, 0, 15000);
	...
	for ( unsigned long long int i=0; i<nEntries; i++ )
    {
        ...
        if(tdcIC >=690 && tdcIC <=704)
        {
            ...
            {
                cout << "HERE!!!!!!!!!!!!!!!!" << endl;
                saved_timestamp = saved_timestamp + prev_timestamp;
            }

            event_time = floor(timeStamp/10000000.0) + saved_timestamp;

            // cout << event_time << setw(10) << prev_timestamp << endl;

            prev_timestamp = floor(timeStamp/10000000.0);



            rate_hist->Fill(floor(event_time/ 1000));
        }


        if(runNumber > prev_runNumber)
        {
            run_marker_hist->SetBinContent(floor(event_time/ 1000), runNumber*1000);
        }
                ...
                if(SX3Det[j] <= 11 && SX3Strip[j] <= 3 && tdcIC >= 900 && tdcIC <= 1300) //No weird events with wrong channels make it through
                {
                    ...
                    if(xtalsMul >= 1 && xtalsMul <= 3 && SX3Upstream[j]==1)
                    {
                        for(int k=0; k<xtalsMul; k++)
                        {
                            ...
                            if(SX3Upstream[j] == 1) gammaEx_matrixSX3->Fill(3593.0 - qValue, xtals_edop[k]);
                    ...
                    if(BB10Mul >= 1)
                    {
                        for(int l=0; l<BB10Mul; l++)
                        {
                            ...
                            if(DS_deuterons->IsInside(BSX3_En, BB10_En) && ( SX3Det[j]==0 || SX3Det[j]==1 || SX3Det[j]==2 || SX3Det[j]==3 || SX3Det[j]==4 ||SX3Det[j]==7 ||SX3Det[j]==8 ||SX3Det[j]==9 ||SX3Det[j]==10) )
                            {
                                kinematics_deuteron_PID->Fill(angle, BSX3_En+BB10_En);

                                total_elastics_count++;

                                elastics_rate_count[runNumber]++;

                                elastics_rate_hist->Fill(floor(event_time/ 1000));
                ...
                if(QQQ5Det[j] <= 4 && QQQ5Ring[j] <= 31 && tdcIC >= 900 && tdcIC <= 1300) //Se82_locus_IC->IsInside(icE, icdE)
                {
                    ...
                    if(xtalsMul >= 1 && xtalsMul <= 3 && QQQ5Upstream[j] == 1)
                    {
                        for(int k=0; k<xtalsMul; k++)
                        {
                            ...
                            if(QQQ5Upstream[j] == 1) gammaEx_matrixQQQ5->Fill(3593.0 - qValue, xtals_edop[k]);
    ...
    double elastics_IC_ratio[200] = {0};
    ...
    cout << endl;
    cout << "Total elastic deuterons:  " << total_elastics_count << endl;
    cout << endl;
    ...
    //Scaling rate histogram to represent /s
    rate_hist->Scale(327.68);

    Si_rate_comparrison->Write();
    elastics_IC_ratio_vs_RunNumber->Write();
    rate_hist->Write();
    elastics_rate_hist->Write();
    run_marker_hist->Write();
    //IC_rate_comparrison->Write();
    DS_PID->Write();
    kinematics->Write();
    ic_dE_E->Write();
    ...
    c1->cd(1);
    Si_rate_comparrison->Draw();
    IC_rate_comparrison->Draw("SAME");
    c1->cd(2);
    gamma_aboveSn->Draw();
    gamma_belowSn->Draw("SAME");
