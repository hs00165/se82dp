                ...
                if (word != 0xffffffff) {
                    ...
                    for(int k = 0; k < readChannel.size(); k++) {
                        ...
                        if(channel <= 128 && adc > QQQThreshold) { // QQQ5 upstream front (rings)
                            ...
                            SuperX3Back hit = {channel, detector, channel - 405 - (detector - 4)*4, adc};
                            SX3dBack_.push_back(hit);
                        ...
                        } else if(channel > 672 && channel <= 684 && adc > QQQThreshold) { // QQQ5 downstream back (sectors)
                            int detector = static_cast<int>((channel - 573)/4);
                            QQQ5Sector hit = {channel, detector, channel - 573 - detector*4, adc};
                            QdSector_.push_back(hit);
                        } else if(channel > 576 && channel <= 672 && adc > QQQThreshold) { // QQQ5 downstream front (rings)
                        ...
                        } else if(channel > 704 && channel <= 736 && adc > ICTrackingThreshold) { // IC x
                            int wire = channel - 705;
                            ICTracking hit = {true, wire, adc};
                            ICx_.push_back(hit);
                        } else if(channel > 736 && channel <= 768 && adc > ICTrackingThreshold) { // IC y
                            int wire = channel - 737;
